...
for (let i = 0; i < buttonLength; i++) {
    ...
    document.getElementsByTagName("button")[i].addEventListener("click", function (event) {
        if (event.target.textContent ===
            ...
            displayAnswer("不正解", questionContents[questionLength].answer, answerBox);
        }
        questionLength++;
...
function displayAnswerInParagraph(answerMessage, answerText, answerBox) {
    if (answerMessage === "正解") {
        answerBox.insertAdjacentHTML("beforeend", `<p>第${questionLength + 1}問は${answerMessage}です。`);
    }
    else {
